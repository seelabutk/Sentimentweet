...
$(document).ready(function(){
    ...
    $(document).on("click", ".term .fa-times", function(){
    ...
    google.maps.event.addListener(map, 'click', function(event){
        ...
        $("#tweets").html('Loading...');
        $.getJSON("get_tweets.php", { lat: latitude, lng: longitude, search_terms: search_terms}, function(results){
            $("#tweets").html('');
            var tweets_text = '';
            for (i in results)
            {
...
function update_map(terms)
{
    ...
    /* clear current table */
    $("#term_frequency").css("display", "none");
    $("#term_frequency > thead").empty();
    $("#term_frequency > tbody").empty();
    $.getJSON("get_positions.php", { search_terms: terms }, function(results){
        ...
        /* add table headers */
        var headerHtmlStr = "<tr><td></td>";
        for(var t = 0; t < times.length; t++)
        {
            console.log(times[t]);
            headerHtmlStr += "<td>" + times[t] + "</td>";
        }
        headerHtmlStr += "</tr>";
        ...
        for (term in results)
        {
            for (time in results[term])
            {
                ...
                if(timeIdx !== -1)
                {
                    $("#term_frequency > tbody").children().eq(termIdx).children().eq(timeIdx+1).css("background-color", freq_color(results[term][time].length));
                }
        ...
        if(terms.length > 0)
        {
            $("#term_frequency").css("display", "block");
        }
