...
$(document).ready(function(){
    ...
    search_terms = [];
    layer = null;
    color_map = colorbrewer.RdYlGn[9];
    ...
    $("#colorblind-btn").on("click", function(){
        if (color_map == colorbrewer.RdYlGn[9])
        {
            color_map = colorbrewer.RdBu[9];
            $("#colorblind-btn").css("font-weight", "700");
            $("#colorblind-btn i").removeClass("fa-circle-o");
            $("#colorblind-btn i").addClass("fa-check-circle");
        }
        else
        {
            color_map = colorbrewer.RdYlGn[9];
            $("#colorblind-btn").css("font-weight", "400");
            $("#colorblind-btn i").removeClass("fa-check-circle");
            $("#colorblind-btn i").addClass("fa-circle-o");
        }
        update_map(search_terms, undefined);
    });
    ...
    google.maps.event.addListener(map, 'click', function(event){
        ...
        $("#tweets").html('Loading...');
        $.getJSON("get_tweets.php", { lat: latitude, lng: longitude, zoom: map.zoom, search_terms: search_terms, search_times: time_range }, function(results){
            $("#tweets").html('');
            var tweets_text = '';
            temp_results = results;
            for (i in results)
            {
                tweet_header = '<div class="tweet-header"><img src="' + results[i]['profile_image_url'] + '"/>'
                     + '<span class="tweet-username">' + results[i]['name'] + '</span>'
                     + ' <span class="tweet-screenname"><a href="http://twitter.com/@' + results[i]['screen_name'] + '">(@' + results[i]['screen_name'] + ')</a></span>&nbsp;'
                     + ' </div><br>';
                tweets_text += tweet_header + '<span class="tweet-text">' + results[i][1] + '</span><hr>';
                /*$("<div>")
                    .appendTo("#tweets")
                    .addClass("tweet")
                    .text(results[i][1]);*/
            ...
            if (results.length > 0)
            {
                ...
                google.maps.event.addListener(infowindow, 'domready', function() {
                    ...
                    var text = $(".gm-style-iw > div > div").html();
                    //text = text.replace(/([@#][^ ]+)/g, '<a href="http://twitter.com/$1">$1</a> ');
                    //$(".gm-style-iw > div > div").html(text);
                    $(".gm-style-iw > div > div a").css({'text-decoration': 'none'});
                    var back = $(".gm-style-iw").prev();
                    back.children(":nth-child(2)").css("display", "none");
                    back.children(":nth-child(4)").css("display", "none");

                    var close_btn = $(".gm-style-iw").next();
                    close_btn.css({
                        opacity: '1', // by default the close button has an opacity of 0.7
                        right: '5px', top: '-13px', // button repositioning
                        border: '7px solid #eee', // increasing button border and new color
                        'border-radius': '5px 5px 0 0' // circular effect
                    });
                    back.children(":nth-child(3)").attr("style", function(i, s){return s+'top: 120px'});
                    back.children(":nth-child(3)").children(":nth-child(1)").children(":nth-child(1)").css({'background-color': '#eee'})
                    back.children(":nth-child(3)").children(":nth-child(2)").children(":nth-child(1)").css({'background-color': '#eee'})
                });
            }
        });
    });
...
{
    $("#colorblind-btn").css("display", "block");
    ...
    if(time !== undefined)
    {
      ...
      if (Date.parse(time[0]) > Date.parse(time[1])) {
        var tmp = time_range[0];
        time_range[0] = time_range[1];
        time_range[1] = tmp;
      }

      time_range = [
        time_range[0] + " 00:00:00",
    ...
    $.getJSON("get_positions.php", { search_terms: terms, search_times: time_range }, function(results){
        $.getJSON("get_heatmap.php", { search_terms: terms, search_times: time_range }, function(heatmap){
            ...
            {
                headerHtmlStr += "<th><div><span onmousedown='th_mouse_down(event, this);' onmouseup='th_mouse_up(event, this);' onmousemove='th_mouse_move(event, this);'>" + times[t] + "</div></span></th>";
            }
            if(time_range !== "")
            {
            ...
            for(term in results)
            {
                var bodyHtmlStr = "<tr><td><i class='fa fa-times'/>&nbsp;<span>" + term + "</span></td>";
                for(time in times)
                {
                    bodyHtmlStr += "<td></td>";
                }
...
var tr = [undefined, undefined];
var tr_el = [undefined, undefined];
function th_mouse_down(e, el)
{
  if(e.which !== 1) {
    return;
  }
...
function th_mouse_up(e, el)
{
  ...
  if(tr[0] !== undefined)
  {
    update_map(search_terms, tr);
  }

  tr = [undefined, undefined];
  tr_el = [undefined, undefined];
}

function th_mouse_move(e, el)
{
  if(e.which !== 1) {
    return;
  }
  ...
  if(tr_el[0] !== undefined && tr_el[1] !== undefined)
  {
    var paint = false;
    var paintChanged = false;
    $("#term_frequency span").each(function(idx, el) {
      if(!paint && (el === tr_el[0] || el === tr_el[1]))
      {
        paint = true;
        paintChanged = true;
      }
      ...
      if((tr_el[0] === tr_el[1]) || (paint && !paintChanged && (el === tr_el[0] || el === tr_el[1])))
      {
        paint = false;
      }
